namespace DefiningClassesOne
{
    using System;
    using System.Collections.Generic;
    using System.Linq;

    public struct Point3D
    {
        //if you dont have a constructor
        //private static readonly Point3D start = new Point3D() { X = 0, Y = 0, Z = 0 };

        private static readonly Point3D start = new Point3D(1, 2, 3);

        public Point3D(double x, double y, double z)
            :this()
        {
            this.X = x;
            this.Y = y;
            this.Z = z;
        }

        public static Point3D Start 
        {
            get
            {
                return start;
            } 
        }
        public double X { get; set; }
        public double Y { get; set; }
        public double Z { get; set; }

        public static Point3D Parse(string text)
        {
            //"Point (0, 0, 3)"
            int openPar = text.IndexOf("(");
            double[] coord = text
                .Substring(openPar + 1, text.Length - 2 - openPar) // or new[] { ",  "}
                .Split(new[] { ' ', ',' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(x => double.Parse(x))
                .ToArray();

            //if you dont have a constructor in the struct Point3D
           // return new Point3D() { X = coord[0], Y = coord[1], Z = coord[2]};

            return new Point3D(coord[0], coord[1], coord[2]);
        }

        public override string ToString()
        {
            return string.Format("Point ({0}, {1}, {2})", this.X, this.Y, this.Z);
        }
    }
}
